...
if __name__ == '__main__':
	...
	parser.add_argument('-t', '--time', action='store_const', const=12)
	...
	with h5py.File(data_path, 'r') as f:
		nchans = f['cfg_PAINT_cond']['channum'][0]
		nsources = f['cfg_PAINT_cond']['sourcenum'][0]
		nfreqs = f['cfg_PAINT_cond']['ERSPfreq'].size
	...
	for i, jobtype in enumerate(['channels', 'freq_channels', 'freq-sources', 'sources'])
			...
			for i in range(jobnums[i]):
				...
				with open(sbname, 'w') as sb:
					...
					if 'cori'.encode() in hostname:
						sb.write('srun -C haswell python3  %s/%s %s %s %s %d' 
							% (script_dir, data_path, jobtype, i))
					else:
